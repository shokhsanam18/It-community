...
export const Questions = () => {
  ...
  return (
    <div
      className={`min-h-screen px-4 py-8 flex flex-col items-center transition-all duration-300 bg-gradient-to-b from-[#e4e7e6] to-[#b0ddaa] ${showFireworks ? "overflow-hidden" : ""
        }`}
    >
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option) => {
            ...
            return (
              <label
                key={option}
                className={`block px-4 py-2 border rounded-lg cursor-pointer ${isSelected ? "bg-green-100 border-green-500" : "bg-white border-gray-200"
                  }`}
              >
                <input
                  type="radio"
